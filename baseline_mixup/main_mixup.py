...
def train_val(net, optimizer, scheduler, criterion, epoch , train_loss_list, train_acc, name):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            writer.add_scalar('mixup_'+str(name)+"/loss_train", loss.item(), epoch)
            writer.add_scalar('mixup_'+str(name)+"/acc_train", 100. * correct / total, epoch)  
...
def val(net, optimizer, scheduler, criterion, epoch , val_loss_list, val_acc, name):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(valloader):
            ...
            #              % (test_loss/(batch_idx+1), 100.*correct/total, correct, total))
            writer.add_scalar('mixup_'+str(name)+"/loss_val", loss.item(), epoch) 
            writer.add_scalar('mixup_'+str(name)+"/acc_val", 100.*correct/total, epoch)  
